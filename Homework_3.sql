...
DROP TABLE IF EXISTS first_tbl;
CREATE TABLE tbl1 (
	id SERIAL, 
	quantity_friends BIGINT UNSIGNED,
    `description` TEXT,
    FOREIGN KEY (from_user_id) REFERENCES users(id)
);
...
CREATE TABLE tbl2 (
	id SERIAL, 
	quantity_group BIGINT UNSIGNED PRIMARY KEY,
    `description` TEXT,
    FOREIGN KEY (from_user_id) REFERENCES users(id)
);
...
CREATE TABLE tbl2 (
	id SERIAL, 
    subscribers_on_group BIGINT UNSIGNED,
    FOREIGN KEY (number_group) REFERENCES second_tbl(quantity_group),
    FOREIGN KEY (from_user_id) REFERENCES users(id)
);
